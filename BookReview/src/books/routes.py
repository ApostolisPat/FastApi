...
from src.books.service import BookService #Import the databaseService
...
async def get_book(book_uid: str, session: AsyncSession = Depends(get_session)) -> dict:
    ...
    if book:
        ...
        raise HTTPException(status.HTTP_404_NOT_FOUND, "book with id {book_uid} not in database.")
...
async def update_book(book_uid: str, book_update_data: BookUpdateModel, session: AsyncSession = Depends(get_session)) -> dict:
    ...
    if updated_book is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, detail = "book with id {book_uid} not in database.")
    else:
        return updated_book 
